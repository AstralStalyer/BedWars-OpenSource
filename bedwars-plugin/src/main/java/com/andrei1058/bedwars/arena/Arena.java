...
public class Arena implements IArena {
    ...
    public boolean addPlayer(Player p, boolean skipOwnerCheck) {
        ...
        if (status == GameState.waiting || (status == GameState.starting && (startingTask != null && startingTask.getCountdown() > 1))) {
            ...
            for (Player on : players) {
                on.sendMessage(color(Objects.requireNonNull(Objects.requireNonNull(LuckPermsProvider.get().
                        getUserManager().getUser(p.getName())).getCachedData().getMetaData().getPrefix()).
                        substring(0, 2) + p.getDisplayName() + " §e加入了游戏(§b" + (getPlayers().size()) + "§e/§b" +
                        (getMaxPlayers()) + "§e)！"));
            }
    ...
    public void removePlayer(@NotNull Player p, boolean disconnect) {
        ...
        for (Player on : getPlayers()) {
            on.sendMessage(color(Objects.requireNonNull(Objects.requireNonNull(LuckPermsProvider.get().
                    getUserManager().getUser(p.getName())).getCachedData().getMetaData().getPrefix()).
                    substring(0, 2) + p.getDisplayName() + " §e已退出！"));
        }
