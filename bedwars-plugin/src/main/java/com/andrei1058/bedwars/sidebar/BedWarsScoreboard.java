...
public class BedWarsScoreboard {
    ...
    private void setStrings(@NotNull List<String> strings) {
        ...
        for (String current : strings) {
            ...
            current = current
                    .replace("{server_ip}", "StarryStudios")
    ...
    private SidebarLine getTeamListText(String path, Player targetPlayer) {
        ...
        for (String string : Language.getList(getPlayer(), path)) {
                strings.add(string.replace("{vPrefix}", BedWars.getChatSupport().getPrefix(targetPlayer))
                        ...
                        .replace("{cPrefix}", LuckPermsProvider.get().getUserManager().getUser(targetPlayer.getName()).getCachedData().getMetaData().getPrefix().substring(0, 2))
